...
public class GraphBiPartite {
    ...
    public static boolean isBipartite(int[][] graph) {
        ...
        for (int i = 0;
             i < graph.length; i++) {   //This graph might be a disconnected graph. So check each unvisited node.
    ...
    public static boolean dfs(int[][] graph, int[] color, int curColor, int node) {
        if (color[node] != 0) {
        ...
        for (int neighbor : graph[node]) {
            if (!dfs(graph, color, -curColor,
                    neighbor)) {  // If this node hasn't been colored, Color it with a different color;
                return false;
            }
        }
        return true;
