...
public class Fenwick2D {
    ...
    public Fenwick2D(int[][] matrix) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                // Another change that happens in the case of 2D bit is,
                // the partial sums that we store now are corresponding to
                // partial sum of the non-overlapping sub-rectangles or sub-matrix present in the matrix.
