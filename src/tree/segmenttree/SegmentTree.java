 ...
 *         intervals. Write a program to support these operations for sum
 *         createSegmentTree(int arr[]) - create segment tree query(int
 *         segment[], int startRange, int endRange) - query in this range
 * 
 *         Similar segment trees can be created for min or max.
 * 
 *         Time complexity to create segment tree is O(nlogn) Space complexity
 *         to create segment tree is O(nlogn) Time complexity to search in
 *         segment tree is O(logn)
 * 
 *         References
