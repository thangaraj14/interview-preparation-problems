...
public class UberSeatingProblem {
    ...
    /**
     * Flow goes like this:
     *      step 1: let's say democrat 1 comes, it gets lock on object, increases count, and goes directly to else state, releases lock and checks for semaphore, since it's
     *              not there it will be in blocked state (ready to proceed once acquiring semaphore)
     *      step 2: let's say 3 more democrats are waiting, fourth thread comes and goes into first if and releases 3 semaphores, the 3 previous waiting thread goes and
     *              wait's for current thread to join at the barrier, once it comes all goes into seating
     *      step 3: if more that 2 republicans are waiting and current democrat count is 2, this condition releases 1 democrat and 2 republican semaphore
